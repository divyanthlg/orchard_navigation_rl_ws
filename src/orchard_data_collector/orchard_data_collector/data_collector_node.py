"""
Orchard BC Data Collector Node — v0.6 (Laptop Edition)
========================================================
Runs on the LAPTOP. Subscribes to the Warthog's camera + filtered odometry
topics over WiFi (via ROS 2 discovery) and saves pairs to the laptop's disk.

Reads linear and angular velocities directly from the Odometry message's
twist.twist field (already computed by the robot's EKF / state estimator).

Key features:
- Service-based start/stop (starts PAUSED)
- Records ALL frames (stationary filter off)
- Appends to existing labels.csv
- BEST_EFFORT QoS on both subscribers
- Uses message_filters.ApproximateTimeSynchronizer for topic sync
- Saves directly to the laptop's project data folder

Services:
    ~/toggle_recording (std_srvs/SetBool)
    ~/status           (std_srvs/Trigger)
"""
...
class DataCollectorNode(Node):
    ...
    def __init__(self):
        ...
        # ── Sensor QoS (BEST_EFFORT — matches Warthog publishers) ───────
        ...
        # Both Image and Odometry have headers — no allow_headerless needed
        ...
        self.get_logger().info('╔══════════════════════════════════════════════════╗')
        self.get_logger().info(f'║   Orchard BC Collector (Laptop) — {state:<13s} ║')
        self.get_logger().info('╠══════════════════════════════════════════════════╣')
        ...
        self.get_logger().info(f'║  Sync slop:      {self.sync_slop:.3f}s  (queue={self.sync_queue_size})')
        ...
        self.get_logger().info('║    ros2 service call /data_collector/\\           ║')
        self.get_logger().info('║      toggle_recording std_srvs/srv/SetBool \\     ║')
        self.get_logger().info('║      "{data: true}"   (or false to stop)         ║')
        self.get_logger().info('╚══════════════════════════════════════════════════╝')
        self.get_logger().info('')
    ...
    def _synced_cb(self, image_msg: Image, odom_msg: Odometry):
        ...
        # Read velocities directly from the filtered odometry
