...
class BCStatusDisplay(Node):
    def __init__(self):
        super().__init__('bc_status_display')
    ...
    def _source_cb(self, msg): self.active_source = msg.data
    def _cmd_cb(self, msg):    self.cmd_vel    = msg
    ...
    def _display(self):
        ...
        print(
            ...
            f'bc_policy: lin={p_lin:+.3f} ang={p_ang:+.3f}  ',
            end='', flush=True)


def main(args=None):
    rclpy.init(args=args)
    node = BCStatusDisplay()
    print('\n  BC Policy — Live Status\n  ───────────────────────')
    try:
        rclpy.spin(node)
