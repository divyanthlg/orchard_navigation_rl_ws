# orchard_bc_training — Cheat Sheet (v0.7.4)

Workspace: `~/ros2/orchard_navigation_rl_ws`

**v0.7.4 change:** each time recording is started the collector creates a
fresh, timestamped session folder instead of appending to a shared CSV.

```
data/raw/
└── sessions/
    ├── session_20250510_143201/
    │   ├── images/          ← frame_000000.png … frame_NNNNNN.png
    │   └── labels.csv       ← filename, stamp, odom_stamp, linear_vel, angular_vel
    ├── session_20250510_151042/
    │   ├── images/
    │   └── labels.csv
    └── …
```

`build_cache` discovers all `session_*` folders automatically, merges them
in chronological order (folder names are timestamp-sorted), and writes the
flat `latents.npy` + `index.csv` exactly as before.
`train`, `datasets`, and `bc_deploy` are **completely unchanged**.

**v0.7.3 change:** `labels.csv` records both image and odom timestamps.
...
training); `odom_stamp` is the odom header timestamp (diagnostic).

**v0.7.2 change:** image input is now `sensor_msgs/CompressedImage`.
Default topic: `/sensors/camera_0/color/compressed`.
...
```
  [1] bc_collect.launch.py ──► data/raw/sessions/session_<YYYYMMDD_HHMMSS>/
  [2] build_cache          ──► data/raw/latents.npy + index.csv  (merged)
...
```bash
# Terminal A — start the collector (waits in PAUSED state)
ros2 launch orchard_bc_training bc_collect.launch.py

# Terminal B — start a recording session (new timestamped folder created)
...
# Start another session — a second folder is created automatically
ros2 service call /bc_data_collector/toggle_recording \
    std_srvs/srv/SetBool "{data: true}"
# drive again...
ros2 service call /bc_data_collector/toggle_recording \
...
Each `toggle … true` call creates a new `session_<YYYYMMDD_HHMMSS>/` folder.
Frame filenames always start from `frame_000000.png` inside each session —
there is no global frame counter to manage.
The toggle is **idempotent**: sending `true` while already recording, or
`false` while already stopped, is silently ignored.

**Key launch args:**

| Arg | Default | Notes |
|---|---|---|
| `data_root` | `…/data/raw` | Parent of `sessions/` subdirectory |
| `save_rate_hz` | `2.0` | Don't change (must match training) |
| `auto_start` | `false` | Set `true` to begin recording immediately |
| `image_topic` | `/camera/…/compressed` | Override if your topic differs |

```bash
# Custom data root
ros2 launch orchard_bc_training bc_collect.launch.py \
    data_root:=/mnt/ssd/orchard_data
...
Walks every `sessions/session_*/` folder under `data_root`, merges their
`labels.csv` files in chronological order (alphabetical = timestamp order),
then passes every PNG through the frozen VAE once. Writes:

- `<data_root>/latents.npy` — float16, shape `(N_total, 4, 32, 32)`
- `<data_root>/index.csv`   — merged index with sequential `idx`

Re-run whenever new sessions have been collected.
...
| No frames arriving | Topic mismatch. `ros2 topic hz /sensors/camera_0/color/compressed` — if 0 Hz, check the actual topic name or whether the camera driver is publishing the compressed variant. |
...
| `No sessions/ directory found` | `data_root` is wrong, or you haven't collected any data yet. |
| `No session_* folders found` | `sessions/` exists but is empty — toggle recording at least once. |
...
**Subscribed:** `/sensors/camera_0/color/compressed` (CompressedImage),
`/platform/odom/filtered`, `/w200_0100/rc_teleop/cmd_vel`.
...
The flat `data/raw/images/` + `data/raw/labels.csv` layout is replaced by
`data/raw/sessions/session_<ts>/`. If you have existing v0.7.3 data to keep,
wrap it with one command:

```bash
mkdir -p ~/ros2/orchard_navigation_rl_ws/data/raw/sessions/session_legacy
mv ~/ros2/orchard_navigation_rl_ws/data/raw/images \
   ~/ros2/orchard_navigation_rl_ws/data/raw/sessions/session_legacy/
mv ~/ros2/orchard_navigation_rl_ws/data/raw/labels.csv \
   ~/ros2/orchard_navigation_rl_ws/data/raw/sessions/session_legacy/
```

Then re-run `build_cache` as normal — it will pick up `session_legacy` along
with any new sessions.

### Launch arg change

`bc_collect.launch.py` no longer accepts `image_dir` or `labels_file`.
Use `data_root` (defaults to `…/data/raw`) instead.

### Unchanged files

`bc_deploy.launch.py`, `bc_policy_node.py`, `bc_viz_node.py`,
`bc_cmd_vel_mux_node.py`, `bc_status_display.py`, `train.py`,
`datasets.py`, `models.py`, `vae_resolve.py` — **no changes**.

---

## Migration notes (v0.7 → v0.7.3)

- `labels.csv` gained `odom_stamp` column. Pre-v0.7.3 CSVs (4 columns) are
  still readable by `build_cache` — it fills `odom_stamp = stamp`.
- Image type: `sensor_msgs/Image` → `sensor_msgs/CompressedImage`.
